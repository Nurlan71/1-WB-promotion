...
function get_advList() {
  ...
  if (isConnected) {
    try {
      const advertApiKey = getAPIStatus('emonkey_advert');
      WildberriesAPI.initialize({ advert: advertApiKey });

      const countData = WildberriesAPI.advert.getCampaignCount();

      const advListSheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('📝 Список РК');
      if (!advListSheet) throw new Error("Sheet '📝 Список РК' not found.");

      const typeMap = { 4: "Каталог", 5: "Карточка товара", 6: "Поиск", 7: "Рекомендации", 8: "Автоматическая", 9: "Поиск + каталог" };

      const output = [['Тип кампании', 'Количество']];
      if (countData && countData.length > 0) {
        countData.forEach(item => {
          output.push([ typeMap[item.type] || `Неизвестный тип (${item.type})`, item.count ]);
        });
      }

      advListSheet.clearContents();
      advListSheet.getRange(1, 1, output.length, output[0].length).setValues(output);
      Logger.log("Список РК (количество) успешно обновлен.");

    } catch (error) {
      Logger.log('Ошибка при выполнении get_advList: ' + error.message);
      SpreadsheetApp.getUi().alert('Ошибка при получении данных: ' + error.message);
    }
  }
}
...
function get_adverts() {
  ...
  if (isConnected) {
    try {
      ...
      // Эта функция из кода пользователя получает лист и ID кампаний для обработки
      var { advertSheet, campaignIds } = Ecommonkey.Wildberries.setupAdvertSheet();

      if (campaignIds && campaignIds.length > 0) {
        const campaignJsonData = WildberriesAPI.advert.getCampaignList(campaignIds);

        // ПРЕОБРАЗОВАНИЕ ДАННЫХ: т.к. я не вижу код Ecommonkey.Wildberries.fetchCampaignData,
        // я преобразую ответ от API в 2D-массив самостоятельно, выбрав основные поля.
        // Порядок и набор колонок могут потребовать корректировки под вашу таблицу.
        ...
        Logger.log("Не найдены ID кампаний для обработки в get_adverts.");
      }
    } catch (error) {
      Logger.log('Ошибка при выполнении get_adverts: ' + error.message);
      SpreadsheetApp.getUi().alert('Ошибка при получении данных: ' + error.message);
    }
  }
}
...
function get_words_clust() {
  ...
  if (isConnected) {
    try {
      ...
      // Get the sheet object from the user's helper
      var advClustSheet = Ecommonkey.Wildberries.setupAdvClustSheet();

      // Call the library to get the cluster data
      ...
      advClustSheet.clearContents();
      advClustSheet.getRange(1, 1, output.length, output[0].length).setValues(output);
      Logger.log(`Кластеры для кампании ${campaignId} успешно загружены.`);

    } catch (error) {
      Logger.log('Ошибка при выполнении get_words_clust: ' + error.message);
      SpreadsheetApp.getUi().alert('Ошибка при получении данных: ' + error.message);
    }
  }
}
...
function setexcludedFormulas() {
  try {
    Ecommonkey.Wildberries.setexcludedFormulas();
  } catch (error) {
    Logger.log("Ошибка при вызове setexcludedFormulas: " + error.message);
  }
}
...
function set_excluded() {
  ...
  if (isConnected) {
    try {
      const advertApiKey = getAPIStatus('emonkey_advert');
      const credentials = Ecommonkey.Wildberries.getCampaignId();
      if (!credentials) return;
      const campaignId = credentials.campaignId;

      // ПРЕДПОЛОЖЕНИЕ: Существует функция-хелпер для получения минус-слов из таблицы.
      const keywordsToExclude = Ecommonkey.Wildberries.getKeywordsToExclude();

      WildberriesAPI.initialize({ advert: advertApiKey });
      WildberriesAPI.advert.setExcludedKeywords(campaignId, keywordsToExclude);

      Logger.log(`Запрос на установку минус-фраз для кампании ${campaignId} отправлен.`);
      Browser.msgBox("Минус-фразы успешно обновлены.");

    } catch (error) {
      Logger.log('Ошибка при установке минус-фраз: ' + error.message);
...
function delete_excluded() {
  ...
  if (isConnected) {
    try {
      ...
      const response = ui.alert('Подтверждение удаления', 'Вы точно хотите удалить все минус слова и добавить их в плюс фразы?', ui.ButtonSet.YES_NO);
      if (response != ui.Button.YES) return;

      WildberriesAPI.initialize({ advert: advertApiKey });
      // Для удаления всех минус-слов, передаем пустой массив
      WildberriesAPI.advert.setExcludedKeywords(campaignId, []);

      ui.alert('Все минус-фразы были успешно удалены.');

    } catch (error) {
      Logger.log('Ошибка при удалении минус-фраз: ' + error.message);
      SpreadsheetApp.getUi().alert('Ошибка: ' + error.message);
    }
  }
}
...
function advPause() {
  ...
  if (isConnected) {
    try {
      ...
      var response = Browser.msgBox("Вы точно хотите поставить кампанию на паузу?", Browser.Buttons.YES_NO);
      if (response === 'no') return;

      WildberriesAPI.initialize({ advert: advertApiKey });
      WildberriesAPI.advert.pauseCampaign(campaignId);

      Browser.msgBox("Запрос на паузу кампании отправлен успешно.");
    } catch (error) {
      Logger.log('Ошибка при приостановке кампании: ' + error.message);
      SpreadsheetApp.getUi().alert('Ошибка: ' + error.message);
    }
  }
}
...
function advStart() {
  ...
  if (isConnected) {
    try {
      ...
      Browser.msgBox("Запрос на запуск кампании отправлен успешно.");
    } catch (error) {
      Logger.log('Ошибка при запуске кампании: ' + error.message);
      SpreadsheetApp.getUi().alert('Ошибка: ' + error.message);
    }
  }
}
...
function get_advBalance() {
  ...
  if (isConnected) {
    try {
      ...
      // ПРЕДПОЛОЖЕНИЕ: Ecommonkey.Wildberries.handleBalanceResponse может обработать распарсенный JSON
      Ecommonkey.Wildberries.handleBalanceResponse(balanceData);
      Logger.log('Баланс успешно получен и обработан.');
    ...
    Logger.log('Нет соединения с Wildberries');
  }
}
...
function get_advBudget() {
  ...
  if (isConnected) {
    try {
      ...
      Ecommonkey.Wildberries.updateBudgetSheet(budgetData);
      Logger.log(`Бюджет для кампании ${campaignId} успешно получен.`);

    } catch (error) {
      Logger.log("Произошла ошибка при получении бюджета: " + error.message);
...
function advStop() {
  ...
  if (isConnected) {
    try {
      ...
      Browser.msgBox("Запрос на завершение кампании отправлен успешно.");
    } catch (error) {
      Logger.log('Ошибка при завершении кампании: ' + error.message);
      SpreadsheetApp.getUi().alert('Ошибка: ' + error.message);
    }
  }
}
...
function adv_cpm() {
  ...
  try {
    ...
    const cpm = Ecommonkey.Wildberries.promptForCpm();
    if (cpm === null) return;

    const nmRaw = Browser.inputBox("Введите артикулы через запятую:", Browser.Buttons.OK_CANCEL);
    if (nmRaw === 'cancel') return;
    const nmList = nmRaw.split(',').map(str => parseInt(str.trim(), 10)).filter(nm => !isNaN(nm));
    if (nmList.length === 0) {
      Browser.msgBox("⛔ Не введено ни одного корректного артикула.");
      return;
    }

    WildberriesAPI.initialize({ advert: advertApiKey });
    WildberriesAPI.advert.setCampaignCpm(campaignId, cpm, nmList);

    Browser.msgBox(`Запрос на установку CPM=${cpm} для кампании ${campaignId} отправлен.`);
  } catch (error) {
    Logger.log('Ошибка при установке CPM: ' + error.message);
    SpreadsheetApp.getUi().alert('Ошибка: ' + error.message);
  }
}
...
function get_adv_booster() {
  ...
  if (isConnected) {
    try {
      ...
      // Black box to get payload and sheet object
      var campaignData = Ecommonkey.Wildberries.getCampaignData();
      if (!campaignData) {
        Logger.log('Нет данных кампании для get_adv_booster');
        return;
      }
      var advBoostSheet = campaignData.advBoostSheet;
      var payload = campaignData.payload; // Assuming payload is {"nm": [123, 456]}
      var nmIds = payload.nm; // Extract the nmIds from the payload

      WildberriesAPI.initialize({ advert: advertApiKey });
      const response = WildberriesAPI.advert.getBoosterStats(nmIds);

      // Black box to process the response and write to the sheet
      if (response) {
        Ecommonkey.Wildberries.processApiResponseBoost(response, advBoostSheet);
      }
    } catch (error) {
      Logger.log('Ошибка при выполнении get_adv_booster: ' + error.message);
      SpreadsheetApp.getUi().alert('Ошибка при получении данных: ' + error.message);
    }
  }
}
...
function get_stocks() {
  ...
  if (isConnected) {
    try {
      const statisticsApiKey = getAPIStatus('emonkey_statistics');
      WildberriesAPI.initialize({ statistics: statisticsApiKey });
      ...
      const stockData = WildberriesAPI.statistics.getStocks(formattedFromDate, formattedToDate);

      // ПРЕДПОЛОЖЕНИЕ: Существует функция-хелпер для записи данных на лист.
      Ecommonkey.Wildberries.populateStocksSheet(stockData, stocksSheet);

      Logger.log("Данные по остаткам успешно загружены.");

    } catch (error) {
      Logger.log('Ошибка при получении остатков: ' + error.message);
      SpreadsheetApp.getUi().alert('Ошибка при получении данных: ' + error.message);
    }
  }
}
...
function get_stocks_warehouseId() {
  ...
  if (isConnected) {
    try {
      // Примечание: исходный код использовал ключ 'supplies', но это эндпоинт статистики.
      const statisticsApiKey = getAPIStatus('emonkey_statistics');
      WildberriesAPI.initialize({ statistics: statisticsApiKey });

      // ПРЕДПОЛОЖЕНИЕ: Это стандартные параметры для отчета.
      // Возможно, их потребуется настроить под ваши нужды.
      const payload = {
        "groupBy": ["warehouseId", "subjectId"],
        "filter": { "stocks": [">", 0] }
      };

      const downloadData = WildberriesAPI.statistics.getWarehouseStockReport(payload);

      if (downloadData) {
        Ecommonkey.Wildberries.writeToStocksSheetid(downloadData);
      }
    } catch (error) {
      Logger.log('Ошибка при получении отчета по остаткам: ' + error.message);
      SpreadsheetApp.getUi().alert('Ошибка: ' + error.message);
    }
  }
}
...
function get_sales() {
  // Триггеры остаются, так как это логика приложения, а не библиотеки
  ScriptApp.newTrigger("setupPeriodSalesTrigger").timeBased().after(9 * 60 * 1000).create();
  ScriptApp.newTrigger("clearTriggersSales").timeBased().after(30 * 60 * 1000).create();
  ...
  if (isConnected) {
    try {
      ...
      const salesData = WildberriesAPI.statistics.getSales(formattedFromDate, formattedToDate);

      if (salesData && salesData.length > 0) {
        Ecommonkey.Wildberries.processSalesData(salesData, salesSheet);
        Logger.log(`Успешно обработано ${salesData.length} записей о продажах.`);
      } else {
        Logger.log('Данные о продажах не найдены или пустые');
      }
    } catch (error) {
      Logger.log('Ошибка при получении продаж: ' + error.message);
      SpreadsheetApp.getUi().alert('Ошибка при получении данных: ' + error.message);
    }
  }
}
...
function get_orders() {
  // Логика триггеров остается в основном скрипте
  ScriptApp.newTrigger("setupPeriodOrderTrigger").timeBased().after(9 * 60 * 1000).create();
  ScriptApp.newTrigger("clearTriggersOrder").timeBased().after(30 * 60 * 1000).create();
  ...
  if (isConnected) {
    try {
      ...
      // Один вызов библиотеки, который обрабатывает всю пагинацию внутри себя.
      const allOrders = WildberriesAPI.statistics.getAllOrders(formattedFromDate, formattedToDate);
      ...
      if (allOrders.length > 0) {
        ...
        Logger.log("Заказы за указанный период не найдены.");
      }

    } catch (error) {
      Logger.log('Ошибка при выполнении get_orders: ' + error.message);
      SpreadsheetApp.getUi().alert('Ошибка при получении данных: ' + error.message);
    }
  }
}
...
function get_incomes() {
  ...
  if (isConnected) {
    try {
      ...
      const incomeData = WildberriesAPI.statistics.getIncomes(formattedFromDate, formattedToDate);

      console.log(JSON.stringify(incomeData, null, 5));
      Ecommonkey.Wildberries.processIncomeData(incomeData, incomeSheet);

    } catch (error) {
      Logger.log('Ошибка при получении данных о поставках: ' + error.message);
      SpreadsheetApp.getUi().alert('Ошибка при получении данных: ' + error.message);
    }
  }
}
...
function get_DetailByPeriod() {
  ...
  if (isConnected) {
    try {
      ...
      const headers = Ecommonkey.Wildberries.getHeaders();
      Ecommonkey.Wildberries.setHeaders(byPeriodSheet, headers);
      ...
      // Один вызов для получения всех данных отчета
      const allReportData = WildberriesAPI.statistics.getDetailByPeriod(formattedDates.from, formattedDates.to);

      if (allReportData && allReportData.length > 0) {
        // ПРЕДПОЛОЖЕНИЕ: Ecommonkey.Wildberries.parseResponse может обработать полный массив данных
        const arr = Ecommonkey.Wildberries.parseResponse(allReportData);

        if (arr.length > 0) {
            const numRows = arr.length;
            const numCols = arr[0].length;
            if (numRows > (byPeriodSheet.getMaxRows() - 1)) {
              byPeriodSheet.insertRowsAfter(byPeriodSheet.getMaxRows(), numRows - (byPeriodSheet.getMaxRows() - 1));
            }
            byPeriodSheet.getRange(2, 1, numRows, numCols).setValues(arr);
            Logger.log(`Успешно записано ${arr.length} строк отчета.`);
        }
      } else {
        Logger.log('Нет данных в отчете по реализации за указанный период.');
      }
    } catch (error) {
      Logger.log('Ошибка при получении отчета по реализации: ' + error.message);
      SpreadsheetApp.getUi().alert('Ошибка при получении данных: ' + error.message);
    }
  }
}
...
function get_card_list() {
  ...
  if (isConnected) {
    try {
      const contentApiKey = getAPIStatus('emonkey_content');
      WildberriesAPI.initialize({ content: contentApiKey });

      // ПРЕДПОЛОЖЕНИЕ: Этот хелпер возвращает объект настроек для запроса
      const { settings } = Ecommonkey.Wildberries.initializeParametersCardList();

      const allCards = WildberriesAPI.content.getAllCards(settings);

      // ПРЕДПОЛОЖЕНИЕ: Существует хелпер для обработки полного списка карточек
      Ecommonkey.Wildberries.processCardList(allCards);
      Logger.log(`Успешно загружено ${allCards.length} карточек товара.`);

    } catch (error) {
      Logger.log('Ошибка при получении списка карточек: ' + error.message);
      SpreadsheetApp.getUi().alert('Ошибка: ' + error.message);
    }
  }
}
...
function get_list_goods() {
  ...
  if (isConnected) {
    try {
      ...
      // Для получения всех товаров, передаем пустой фильтр
      const goodsList = WildberriesAPI.content.getGoodsList({});

      // ПРЕДПОЛОЖЕНИЕ: Существует хелпер для записи данных на лист
      Ecommonkey.Wildberries.populateGoodsSheet(goodsList, list_goodsSheet);
      Logger.log('Успешно загружен список товаров.');

    } catch (error) {
      Logger.log('Ошибка при получении списка товаров: ' + error.message);
      SpreadsheetApp.getUi().alert('Ошибка при получении данных: ' + error.message);
    }
  }
}
...
function get_coefficient() {
  ...
  if (isConnected) {
    try {
      const suppliesApiKey = getAPIStatus('emonkey_supplies');
      WildberriesAPI.initialize({ supplies: suppliesApiKey });

      const coofListSheet = Ecommonkey.Wildberries.setupCoofListSheet();
      const warehouseIDs = Ecommonkey.Wildberries.getWarehouseIDs();

      const coefficientData = WildberriesAPI.supplies.getCoefficient(warehouseIDs);

      // ПРЕДПОЛОЖЕНИЕ: Существует хелпер для обработки и записи данных.
      Ecommonkey.Wildberries.populateCoefficientSheet(coefficientData, coofListSheet);

    } catch (error) {
      Logger.log('Ошибка при получении коэффициентов: ' + error.message);
      SpreadsheetApp.getUi().alert('Ошибка: ' + error.message);
    }
  }
}
...
function get_stock_list() {
  ...
  if (isConnected) {
    try {
      ...
      const warehouseList = WildberriesAPI.supplies.getWarehouseList();

      // ПРЕДПОЛОЖЕНИЕ: Существует хелпер для обработки и записи данных.
      Ecommonkey.Wildberries.processWarehouseList(warehouseList);

    } catch (error) {
      Logger.log('Ошибка при получении списка складов: ' + error.message);
      SpreadsheetApp.getUi().alert('Ошибка: ' + error.message);
    }
  }
}
...
function unanswered_feedbacks() {
  ...
  if (isConnected) {
    try {
      const feedbacksApiKey = getAPIStatus('emonkey_feedbacks');
      WildberriesAPI.initialize({ feedbacks: feedbacksApiKey });

      const unanswered_feedbacks_sheet = Ecommonkey.Wildberries.setupUnansweredFeedbackSheet();
      const countData = WildberriesAPI.feedbacks.getUnansweredCount();

      // ПРЕДПОЛОЖЕНИЕ: Существует хелпер для обработки и записи данных.
      Ecommonkey.Wildberries.processUnansweredCount(countData, unanswered_feedbacks_sheet);
      Logger.log("Успешно получено количество неотвеченных отзывов.");

    } catch (error) {
      Logger.log('Ошибка при получении кол-ва отзывов: ' + error.message);
      SpreadsheetApp.getUi().alert('Ошибка: ' + error.message);
    }
  }
}
...
function feedbacks(isAnsweredInput) {
  ...
  if (isConnected) {
    try {
      ...
      const jsonResponse = WildberriesAPI.feedbacks.getArchivedFeedbacks(params);
      ...
      Logger.log('Ошибка при получении отзывов: ' + error.message);
      SpreadsheetApp.getUi().alert('Ошибка при получении данных: ' + error.message);
    }
  }
}
...
function feedbacks_all() {
  ...
  if (isConnected) {
    try {
      ...
      const processResponse = (response) => {
        if (response.data && response.data.feedbacks) {
          response.data.feedbacks.forEach(feedback => {
            var row = Ecommonkey.Wildberries.createFeedbackRowAll(feedback);
            allRows.push(row);
          });
        }
      };

      let params = { isAnswered: false, take: 5000, skip: 0, order: 'dateDesc', dateFrom, dateTo };
      processResponse(WildberriesAPI.feedbacks.getArchivedFeedbacks(params));
      ...
      Logger.log('Ошибка при получении всех отзывов: ' + error.message);
      SpreadsheetApp.getUi().alert('Ошибка при получении данных: ' + error.message);
    }
  }
}
    ...
    try {
      ...
      if (feedbackIds && feedbackIds.length > 0) {
        for (let i = 0; i < feedbackIds.length; i++) {
          const id = feedbackIds[i][0];
          const text = responseTextsMatrix[i][0];
          if (id && text) {
            Logger.log(`Отправка ответа на отзыв ID: ${id}`);
            WildberriesAPI.feedbacks.patchFeedback(id, text);
            Utilities.sleep(500);
          }
        }
        Browser.msgBox("Ответы на отзывы успешно отправлены.");
      }
    } catch (error) {
      Logger.log('Ошибка при ответе на отзывы: ' + error.message);
      SpreadsheetApp.getUi().alert('Ошибка при отправке данных: ' + error.message);
    }
  }
}
